...
export class ScrapingService {
  ...
  public async checkTwitchStream(username: string): Promise<{ isLive: boolean; title: string }> {
    try {
      ...
      let isLive = false;
      try {
        await page.waitForSelector('span[data-a-target="live-badge"]', { state: 'visible', timeout: 8000 });
        isLive = true;
      } catch {
        isLive = await page.locator('span:has-text("LIVE")').first().isVisible().catch(() => false);
      }

      let title = '';
      if (isLive) {
        title = await page.locator('p[data-a-target="stream-title"][class*="CoreText"]')
          .first().textContent()
          .then(t => t?.trim() || '')
          .catch(() => {
  ...
  public async checkYouTubeStream(channel: string): Promise<{ isLive: boolean; title: string }> {
    try {
      ...
      let baseUrl = channel.startsWith('UC')
